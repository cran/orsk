...
\end{equation}
 where risk$_0$ is the risk of having a positive outcome in the control or unexposed group (i.e., risk$_0 = \frac{n_{01}}{nctr}$). Formula (\ref{eqn:rr}) can be utilized for both the unadjusted and adjusted odds ratio. The formula can also be employed to approximate the lower and upper limits of the confidence interval. For an interested reader, thus, the formula provides a conversion between the relative ratio and odds ratio from the published data. However, it may not be possible to convert the estimate when risk$_0$ is unknown or cannot be estimated from the data.
...
\item Alternately, if the corresponding risks are increased to 68\% and 85\%, respectively, then the relative risk is 1.24 (95\% confidence interval 1.2-1.29). 
\end{itemize}
...
Since only the five best solutions are shown, one important question remains: are there any less accurate but still acceptable solutions (to be defined in Appendix A) generated in low incidence rates (< $10\%$) scenarios? If the answer is yes, in these low incidence rate scenarios, then the relative risks and odds ratios are very close to each other, and consequently the odd ratio in Table~\ref{tab:tab2} is still a valid approximation to the relative risk. Otherwise, all acceptable solutions are generated from high incidence rate scenarios and poor approximation of the relative risk by the odds ratio can be expected. 
In order to answer the question, a deeper analysis with \pkg{orsk} is provided in Appendix A. This additional analysis suggests that, although the incidence rate is unknown from the published data, there is a clear evidence that the incidence is high. Consequently, the reported odds ratio in Table~\ref{tab:tab2} can potentially overestimate the true risk ratio. 
...
In the situations under consideration it can be expected that there is often no unique solution. As such, the user should carefully review the results. It may be unclear which of the computational results can be taken for further analysis, but this is not unusual for an exploratory study. Alternately, one may reasonably hope that a subject matter expert can provide valuable insights to the situation and may help make a decision.
...
\begin{appendix}
\section{Additional Analysis of Table 2}
The following analysis it to show that for Table~\ref{tab:tab2},
there are no good solutions under the low incidence rate assumption. Suppose the incidence of outcome is rare ($ < 10\%$) for the epidural anesthesia users and non-users (this scenario can generate good approximations of relative risks by odds ratios \citep{Cumm:2009}). 
However, Table~\ref{tab:tab2} doesn't support this assumption. 
Specifically, no solution $(n_{01}, n_{11})$ satisfies the following closeness criteria 
\begin{equation}\label{eqn:cr1}
\begin{aligned} 
&\frac{n_{01}}{nctr} < 0.1, \frac{n_{11}}{ntrt} < 0.1,\\
& \left|{\theta(n_{01},n_{11}) - 2.61}\right| < 0.3,\\
& \left|{\theta_L(n_{01},n_{11}) - 2.25}\right| < 0.3,\\
& \left|{\theta_U(n_{01},n_{11}) - 3.03}\right| < 0.3.
\end{aligned}
\end{equation}
 To prove this assertion, first, it is simple algebra to show that the above criteria imply 
\begin{equation}\label{eqn:cr2}
\begin{aligned} 
&\frac{n_{01}}{nctr} < 0.1, \frac{n_{11}}{ntrt} < 0.1,\\
&\left|\log(\theta(n_{01},n_{11})) - \log(2.61)\right|< -\log(1-0.3/2.61),\\ 
&\left|\log(\theta_L(n_{01},n_{11})) - \log(2.25)\right|< -\log(1-0.3/2.25),\\
&\left|\log(\theta_U(n_{01},n_{11})) - \log(3.03)\right|< -\log(1-0.3/3.03).
\end{aligned}
\end{equation}
Thus, it is sufficient to choose
\begin{equation}
 \delta=(\log(1-0.3/2.61))^2  + (\log(1-0.3/2.25))^2  + (\log(1-0.3/3.03))^2,
\end{equation}
 which can generate solutions satisfying the above constraint (\ref{eqn:cr2}). Finding such solutions can be accomplished by the following \proglang{R} code. However, the output of solutions is null. Among all the solutions, thus, none satisfies the closeness criteria (\ref{eqn:cr1}). 
This completes the proof of the assertion that there are no good solutions under the low incidence rate scenarios. 
<<echo=true>>=
### additional analysis of Table 2
anes <- orsk(nctr=1636, ntrt=2601, a=2.61, al=2.25, au= 3.03, method="grid",
 d= (log(1-0.3/2.61))^2  + (log(1-0.3/2.25))^2  + (log(1-0.3/3.03))^2)
tmp <- subset(anes$res, ctr_risk < 0.1 & trt_risk < 0.1 & 
abs(OR - 2.61) < 0.3 & abs(OR_lower - 2.25) < 0.3 & abs(OR_upper - 3.03) < 0.3)
tmp
@
\end{appendix}
